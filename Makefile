...
install:
	...
	ln -sf /tmp/reforis_diagnostics/reforis_static/reforis_diagnostics /tmp/reforis/reforis_static/
...
lint-web: venv
	$(VENV_BIN)/$(DEV_PYTHON) -m pylint --rcfile=pylintrc reforis_diagnostics
	$(VENV_BIN)/$(DEV_PYTHON) -m pycodestyle --config=pycodestyle reforis_diagnostics
...
clean:
	...
	rm -rf $(JS_DIR)/node_modules/ reforis_static/diagnostics/app.min.js
