...
const TEST_MODULES = {
    modules: [
        {
            module_id: "foobar_module",
            description: "foobar_module_description"
        }
    ]
};
...
describe("<Diagnostics />", () => {
    ...
    it("should render modules and reports", async () => {
        expect(getByText(componentContainer, "Diagnostics"))
            .toBeDefined();

        expect(mockAxios.get)
            .toBeCalledWith("/reforis/diagnostics/api/modules", expect.anything());
        // Response to GET modules
        mockAxios.mockResponse({ data: TEST_MODULES });

        expect(mockAxios.get)
            .toBeCalledWith("/reforis/diagnostics/api/reports", expect.anything());
